...
function AdminSection(){
  ...
  const userType = pathName.includes('manager') ? 'ManagerMenuList' : 'SellerMenuList';
  ...
  const renderList = (menuList) => {
    ...
    mainMenuKeys.map((e, i) => {
      ...
      for (let j = 0; j < subMenuKeys.length; j++) {
        ...
        listItem.push(<li key={subMenuTitle}><Link to={subMenuLink} className="font-normal">{ subMenuTitle }</Link></li>);
      }
      list.push(
          <li key={e} tabIndex="0" className={`collapse ${currentPage(pathArr[2])}`}>
              <input type="checkbox" className="peer" />
              <div className="collapse-title peer-checked:bg-sub peer-checked:text-black">{e}</div>
              <ul className="collapse-content peer-checked:bg-sub peer-checked:text-black">{listItem}</ul>
          </li>
      );
      setMenu(list);
      return list;
